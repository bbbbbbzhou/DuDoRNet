function [net,stats] = train_image2image_dag( ...
    net, imdb, getBatch, varargin)
%This is a modified version of the CNN_TRAIN_DAG file
%
...
% Modified: counters for snapshots
iterCnt = 0;
snapshotStep = numel(subset) / params.snapshotFreq;
snapshotCnt = snapshotStep;
...
for t=1:params.batchSize:numel(subset)
  ...
  % Modified: save a snapshot of the reconstructed image
  snapshotCnt = snapshotCnt + batchSize;
  iterCnt = iterCnt + batchSize;
  if snapshotCnt >= snapshotStep && strcmp(mode,'train')
    params.iterCnt = iterCnt;
    params.takeSnapshot(net, params);
    snapshotCnt = snapshotCnt - snapshotStep;
  end
